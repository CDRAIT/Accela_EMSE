  ...
  for (x in CAPIDS) // for each b1expiration (effectively, each license app) 
    {
	   ...
	   var ThruSuspend = getAppSpecific("Throughput Suspend Sent",capId);
	   dDate = new Date();
throughputyear = String(dDate.getFullYear()-1);
	...
	if((ICContact != "" && ROContact != "") && capstatus  == "Active" && Thru == "CHECKED" && childcount >=1 && Thru == "CHECKED" && Thrureceived == null && Thruwarning == "CHECKED"   && (ThruSuspend == "UNCHECKED" || ThruSuspend == null)  )
	   {
	   temail = ICContact + ";" + ROContact;
			     ...
			     report = generateReport("THROUGHPUT SUSPEND",paramMap,"AirQuality",capId);
		         logDebug("DEBUG: send email Report 1" + report);
				 sendtest = sendNotification("pcapcd@placer.ca.gov",temail,"rmoore@placer.ca.gov;zlee@placer.ca.gov","AQ_THROUGHPUT_SUSPEND",emailParameters,new Array(report),capId);
				 logDebug("DEBUG: Found Facility CustomID = " + customID + ".  Email address for Contact = " + temail);
				 if (sendtest == "true")
				 {
					editAppSpecific("Throughput Suspend Sent","CHECKED",capId);
				 }
	   }
	   if((ICContact != "" && ROContact == "") && capstatus  == "Active" && Thru == "CHECKED" && childcount >=1 && Thru == "CHECKED" && Thrureceived == null && Thruwarning == "CHECKED" && (ThruSuspend == "UNCHECKED" || ThruSuspend == null)  )
	   {
