/*------------------------------------------------------------------------------------------------------/
| Program: batchExpireAtPlanCheckTRPA_Res  Trigger: Batch    Client : Placer County
|
...
var NotificationType = "PLAN CHECK EXPIRED NOTICE";                        // Email notification template name
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("Start of Batch Job: batchExpireAtPlanCheckTRPA_Res.");

    var emailsentCount = ExpireNotice();

    logDebug("INFO: Sent " + emailsentCount + " Emails. System Date : " + new Date(paramsCurrentCompareDt).valueOf());
    logDebug("End of Job: batchExpireAtPlanCheckTRPA_Res, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Residential TRPA Building Permits Expired at Plan Check: " + emailsentCount;
}
...
function ExpireNotice() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && capPerType == paramsAppPerType && exists(capSubType, paramsAppSubTypeArray) && !exists(capStatus, paramsAppStatusArray) && permitExpireDate == null) {
            ...
            var planCheckOrPermit = "at Plan Check on"; // Email text for when expiration date is for Plan Check
