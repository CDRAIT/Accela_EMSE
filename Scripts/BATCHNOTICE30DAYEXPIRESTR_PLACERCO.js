/*------------------------------------------------------------------------------------------------------/
| Program: batchNotice30DayExpireSTR_placerco  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 12/01/2020 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Script is executed daily. Tests for Expiration date is 30 days from today and permit is Active
|
| Criteria:
|   Application is in an active status and has been Issued
|   ASI Permit 'Expiration Date' is 30 days from 'today'
|   appGroup = "ShortTermRental"
|   appPerType = "Short Term Rental"
| 
| Updates: TDunn 10/07/2020 updated email from.  Added 'test' email to line
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var paramsStartDt = aa.date.parseDate(dateAdd(null, 30));                         // Start Date for the batch script to select records on Expiration Date.
var paramsEndDt = aa.date.parseDate(dateAdd(null, 30));                           // End Date for the batch script to select records on Expiration Date.
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			...
			getPrimaryAddressLineParam4Notification(params); // sets $$addressLine$$ as primary address line
			params.put("$$PERMITEXPIREDATE$$", permitExpireDate);
            params.put("$$CUSTOMRECID$$", capIDString);
            params.put("$$RECORDALIAS$$", alias);
            params.put("$$CONTACTEMAIL$$", emailTo);
			params.put("$$SHORTDESC$$", shortDesc);
			//params.put("$$TOTNUMBER$$", thisTOT);
			
			updateAppStatus("Pending Renewal","Record status updated by script");

            // Send notification template email //
			emailTrue = sendNotification(senderEmailAddr,emailTo, emailCC, "NOTICE_STR_PERMIT_ABOUT_TO_EXPIRE", params, null);
			aa.print("Your " + alias + " Permit number: " + capIDString + " will expire on " + permitExpireDate + ". Your email address is " + emailTo + ". Notification sent is " + emailTrue );
            capCount++;
