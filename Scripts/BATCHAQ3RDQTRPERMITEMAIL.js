...
function aboutExpLics() {
    var capCount = 0;
    var expResult = aa.cap.getCapIDsByAppSpecificInfoField("Quarter Billing","3rd qtr");
    ...
    for (i in myExp) // for each b1expiration (effectively, each license app) 
    {
		...
		var childcount = getChildrencount("AirQuality/Stationary Source/Permit to Operate/*",oldcapId);
		 ...
		 for(eachchild in capchildren)
		 {
			 ...
			 if (childcap.getCapStatus() == "ACTIVE" && ROContact != "")
			 {
				 ...
				 sendNotification("pcapcd@placer.ca.gov",ROContact,"RMoore@placer.ca.gov","AQ PERMIT",emailParameters,new Array(report),eachChildCapId);
			 }
			  if (childcap.getCapStatus() == "ACTIVE" && ROContact == "")
			 {
				 paramMap = aa.util.newHashMap();
				 ...
				 addParameter(emailParameters,"$$RECORDALTID$$",customID);
				 report = generateReport("AQ_Permit_Report_To_Disk",paramMap,"AirQuality",eachChildCapId);
				 sendNotification("pcapcd@placer.ca.gov","sfrancis@placer.ca.gov","RMoore@placer.ca.gov","AQ PERMIT",emailParameters,new Array(report),eachChildCapId);
