/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticeAboutToExpireTRPAPermits  Trigger: Batch    Client : Placerco
|
| Version 2.0 - Modified from version 1.0,  03/23/2017 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Script is executed daily. Tests for Expiration dates occuring at
| 14 and at 60 days. Generates notice and sets cap status to About to Expire permit
|
| Criteria:
|   Permit is not closed, expired or already about to expire
|   ASI Expiration date is in 14 or 60 days
|   appGroup = "TRPA"
|   appType  = "Building"
|   appSubType = "Residential","Non-Residential","Multi-Family"
| 
| Updates: TDunn 12/21/2023 converted EMSE 2.0 batchExpirePermitsRes into EMSE 3.0 format.
|          TDunn 12/22/2023 consolidated Res and Comm TRPA scripts into single batch script.
...
|          TDunn 03/06/2024 removed app status update from script   
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var paramsAppSubTypeArray = new Array("Residential", "Non-Residential", "Multi-Family");
...
var newCapStatus = ""; 					                                // New Cap Status that will be applied.
var emailTemplate = "PERMIT NEARING EXPIRATION NOTICE";         				// Email notification template name
var paramsOK = true;
/*------------------------------------------------------------------------------------------------------/
| <===========Main=Loop================>
|
...
for(dd in numDaysArray)
{
	...
	if (paramsOK) {
		...
		logMessage("End of Job: batchNoticeAboutToExpireBLD, Elapsed Time : " + elapsed() + " Seconds.");
		emailText = "Number of TRPA permits about to expire" + expiredCount;
	}
...
function expiringPermit() {
    ...
    for (i in csmArray) {
		...
		//aa.print("Category: " + capPerCategory + ", record number = " + capIDString + ". Expiration date: " + permitExpireDate + ". Cap status: " + capStatus);
...
*/			
			...
			aa.print("applEmail = " + applEmail + "; primeEmail = " + primeEmail + "; owner = " + pOwnerEmail + "; LP = " + profEmail);
			aa.print("To emails: " + vToEmail);
