...
| Batch Job Name: CodeEnfNewWfStaffNotifications
|
...
|
| Criteria: 
|                         AppStatus = See within codes

|   
| Modified By: Abe >> 06/24/2026 - for IT Request# 1675 new workflow for Code Enforcement
|               
|                                
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if (paramsOK) {
    ...
    for (var i in paramsAppSpecInfoLabelArray) {
        if (paramsAppSpecInfoLabelArray[i] == "Citation Issuance Date") {
            ...
            emailContentStr = "The 12-day hold following issuance of a citation on the above referenced case has expired and no appeal has been received. " + "Please move forward with the appropriate action.";
        ...
        if (paramsAppSpecInfoLabelArray[i] == "Nuisance Letter Issuance Date") {
            ...
            paramsStartDt = aa.date.parseDate(dateAdd(null, -30));      //12-day hold after citation issuance date
            paramsEndDt = aa.date.parseDate(dateAdd(null, -30));
            emailContentStr = "The 30-day hold following of a nuisance letter on the above referenced case has expired. " + "Please reinspect to determine if the case remains in violation. ";
        ...
        if (paramsAppSpecInfoLabelArray[i] == "Nuisance Abatement Upheld Date") {
            ...
            paramsStartDt = aa.date.parseDate(dateAdd(null, -30));      //12-day hold after citation issuance date
            paramsEndDt = aa.date.parseDate(dateAdd(null, -30));
            emailContentStr = "The 30-day hold following an upheld abatement hearing on the above referenced case has expired. " + "Please reinspect to determine if the case remains in violation. ";
...
function sendStaffNotifications(getCapResult, paramsAppSpecInfoLabel, vDate, vEmaileSubject, emailContent) {
    ...
    for (var i in csmArray) {
        ...
        if (processFlag && capGroup == paramsAppGroup && capPerType == paramsAppPerType) {
            ...
            addParameter(emailParameters, "$$contentString$$", emailContent); /* Email Body */
