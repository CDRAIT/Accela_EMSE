...
//}
   //136132
    ...
    for (x in CAPIDS) // for each b1expiration (effectively, each license app) 
    {
	   ...
	   var duedate = dateAdd(dateFormatted(iList[iNum].getInvDate().getMonth(),iList[iNum].getInvDate().getDayOfMonth(),iList[iNum].getInvDate().getYear(),""),45); // used for when to generate report / due date below, should be set to 45 11/19/19
	   ...
	   if( Balance > 0  && letter != "CHECKED")
{
				 ...
				 sendtest = sendNotification("pcapcd@placer.ca.gov",temail,"rmoore@placer.ca.gov","AQ 1ST LETTER",emailParameters,new Array(report),capId);
