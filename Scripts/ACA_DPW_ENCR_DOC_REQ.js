/*------------------------------------------------------------------------------------------------------/
| Program : ACA_DPW_ENCR_DOC_REQ.js  
| Event   : ACA_Before
|
...
var docListArray2 = aa.document
  ...
  .getOutput()
  .toArray();
var vPermitType = AInfo["Permit Type"];
...
try {
  ...
  // Checking for New Permit Type document requirement
  ...
  if (matches(capType, "Encroachment")) {
    varCancel = true;
    varShowMessage = true;
    var docCategory = ["Site Plan"];
    varComText =
      "You must attach at least one Site Plan type document prior to moving to the next step in the application process.";
    for (x in docListArray) {
      //if (docListArray[x].getDocCategory() == docCategory) {
      if (exists(docListArray[x].getDocCategory(), docCategory)) {
        varCancel = false;
        varShowMessage = falsse;
        varComText = "";
      }
    }
    // for (x in docListArray2) {
    //   //if (docListArray2[x].getDocCategory() == docCategory) {
    //   if (exists(docListArray2[x].getDocCategory(), docCategory)) {
    //     varCancel = false;
