/*------------------------------------------------------------------------------------------------------/
| Program: batchExpirePermitsTRPA_Res  Trigger: Batch    Client : Placer County
|
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("START", "Start of Batch Job: batchExpirePermitsTRPA_Res.");

    var emailsentCount = ExpireNotice();

    logDebug("Sent " + emailsentCount + " Emails. System Date : " + new Date(paramsCurrentCompareDt).valueOf());
    logDebug("End of Job: batchExpirePermitsTRPA_Res, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Residential TRPA Building Permits Expired: " + emailsentCount;
}
