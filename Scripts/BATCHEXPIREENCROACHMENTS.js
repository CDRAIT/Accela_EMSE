/*------------------------------------------------------------------------------------------------------/
| Program: batchExpireEnroachments  Trigger: Batch    Client : Placer County
|
| Version 1.0 - Base Version. Modified from 'batchEmailExpire' scripts. 08/10/2022, Ebrahim Aftahi Placer County, changed Terry's code
| Abe - 10/20/2022: Fixed the error on records without primary address. function getCapAddress()
|
|
| Designed for EMSE 2.0
|
| Description of Intended Functionality: Script is executed daily. Target is application with Expiration Date of 
| 'today'. Expires permit when criteria are met.
|
| Criteria:
...
// Parameter variables to process records - PublicWorks/Encroachment/NA/NA
var paramsAppGroup = "PublicWorks";
var paramsAppPerType = "Encroachment";
var paramsAppSubTypeArray = new Array("NA", "NA"); //not sure if * or NA
var paramsAppStatusArray = new Array("Expired", "Construction Complete", "Void", "Withdrawn", "DONE", "On Hold");           //App Status criteria exception
...
var NotificationType = "PERMIT_EXPIRED_NOTICE_ENC";                                     // Email notification template name
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("START", "Start of Batch Job: batchExpireEncroachmentPermits.");

    var emailsentCount = ExpireNotice();

    logDebug("Sent " + emailsentCount + " Emails. System Date : " + new Date(paramsCurrentCompareDt).valueOf());
    logMessage("END", "End of Job: batchExpireEncroachmentPermits, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Encroachment Permits Expired: " + emailsentCount;
}
...
function ExpireNotice() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && capPerType == paramsAppPerType && exists(capSubType, paramsAppSubTypeArray) && !exists(capStatus, paramsAppStatusArray)) {
            ...
            //sendNotification(senderEmailAddr, "eaftahi@placer.ca.gov", ccEmailAddr, NotificationType, params, null); // Remark out line above and unremark this line for testing
            capCount++;
        }
    }
