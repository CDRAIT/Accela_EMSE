/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticePendingRenewalSTRplacerco  Trigger: Batch    Client : Placerco
|
...
var paramsStartDt = aa.date.parseDate(dateAdd(null, +28));                         // Start Date for the batch script to select records on Expiration Date.
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			var emailTemplate = "NOTICE_STR_PERMIT_ABOUT_TO_EXPIRE";
