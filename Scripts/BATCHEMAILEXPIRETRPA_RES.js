/*------------------------------------------------------------------------------------------------------/
| Program: batchEmailExpireTRPA_Res  Trigger: Batch    Client : Placer County
|
...
| next 30 days.  Generates an email to contact using Notification Template.
|
| Criteria:
|   Permit is not closed, expired, withdrawn, cancelled.
|   ASI Expiration date is 30 days from 'today'.
|   appGroup = "Building"
|
...
var NotificationType = "PERMIT NEARING EXPIRATION NOTICE";                                        // Email notification template name
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("START", "Start of Batch Job: batchEmailExpireTRPA_Res.");
    ...
    logMessage("END", "End of Job: batchEmailExpireTRPA_Res, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Emails Sent for TRPA Residential Building Permits that are about to Expire: " + emailsentCount;
}
...
function ExpireNotice() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && capPerType == paramsAppPerType && exists(capSubType, paramsAppSubTypeArray) && !exists(capStatus, paramsAppStatusArray)) {
            ...
            // sendNotification(senderEmailAddr, "tdunn@truepointsolutions.com", ccEmailAddr, NotificationType, params, null);
            capCount++;
        }
    }
