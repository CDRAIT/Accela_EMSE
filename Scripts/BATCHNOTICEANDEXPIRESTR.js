/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticeAndExpireSTRplacerco  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 01/22/2021 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Script is executed annually on 03/31/xxxx. 
|
| Criteria:
|   Application is has a status of 'Pending Renewal"
|   ASI Permit 'Expiration Date' 12/31 of the prior year
|   appGroup = "ShortTermRental"
|   appPerType = "Short Term Rental"
| 
| Updates: TDunn 01/23/2021 created.
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var paramsStartDt = aa.date.parseDate(dateAdd(null, -92));                         // Start Date for the batch script to select records on Expiration Date.
var paramsEndDt = aa.date.parseDate(dateAdd(null,-89));                                // End Date for the batch script to select records on Expiration Date.
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			...
			var emailTemplate = "NOTICE_STR_PERMIT_HAS_EXPIRED";
