/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticeAndExpireTRPAPermits  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 01/22/2021 Terry Dunn
|
...
|
| Criteria:
|   Permit is not closed, Expired, VOID, Cancelled
|   ASI Expiration date is today - 1 day
|   appGroup = "TRPA"
|   appType  = "Building"
|   appSubType = "Residential","Non-Residential","Multi-Family"
|   appCategory = "Project","Qualified Exempt","TRPA Review at TRPA"
| 
| Updates: TDunn 12/18/2023 converted EMSE 2.0 batchExpirePermitsRes into EMSE 3.0 format.
|          TDunn 12/19/2023 consolidated Res and Comm Building scripts into single batch script.
...
var emailTemplate = "Permit Expired Notice";            						// Email notification template name
var paramsOK = true;
/*------------------------------------------------------------------------------------------------------/
| <===========Main=Loop================>
|
...
if (paramsOK) {
    ...
    var expiredCount = ExpiredPermit();
    

    logMessage("Expired " + expiredCount + " projects. System Date : " + new Date(paramsCurrentCompareDt).valueOf());
    logMessage("End of Job: batchNoticeAndExpireBLD, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Number of Short Term Rental permits expiring" + expiredCount;
}
...
*/			
            ...
            //emailParameters.put("$$CONTACTEMAIL$$", emailAddress);
            // params.put("$$PERMITADDRESS$$", thisAddress);  	replace in notification with $$addressLine$$
			// params.put("$$WORKDESC$$", thisWorkDesc);   		replace in notification with $$workDesc$$
			// params.put("$$PERMITNAME$$", thisPermitName);	replace in notification with $$capName$$
