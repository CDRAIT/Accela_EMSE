...
|           TDunn    02/22/17-Modified for City of Sacramento list of required attachment categories
|           MMCALEER 05/03/17-Modified for City of Sacramento list of required attachment categories by permit type
|           Keith 2/13/18 added function getDocReqsMFCMP() 
|           Keith 2/13/18 added function getDocReqsMFVEH() 
|           Keith 2/13/18 added function getDocReqsMFDRV() 
|           Keith 08/05/18-Add Function function getDocReqsSTRAPP() for short term rental
|           Keith 11/25/18 added function getDocReqsMFVEHB() 
|           Keith 12/29/18 added function getDocReqsMICRO() 
...
var controlString = "ACA_ONLOAD_ADD_CONTACT"; 	// Standard choice for control
...
|
/-----------------------------------------------------------------------------------------------------*/
var conditionKey = "Application Type"; //ASI field name
...
aa.debug("----EMSE---",conditionValue);
if(cap != null)
{
	
	var asiGroups = cap.getAppSpecificInfoGroups();
	var conditionValue = getFieldValue(conditionKey,asiGroups);
	if(conditionValue != null && ("Internal Combustion Engine (emergency and prime use)" == conditionValue || "Gasoline Dispensing Operations (Retail and Non-Retail)" == conditionValue))
	{
	
		clearPageSectionData("3","2")   //2 is the page number ,and 3 is the step number
...
function wrapPeopleModel(peopleModel,capIdModel){
	...
	mod.setRefContactNumber(peopleModel.getContactSeqNumber()); // Set Reference Contact Number.
	return mod;
}
...
function getDocReqsStreet() {
    requirementArray = new Array();
    missingDocsArray = new Array();

    requirementArray.push("Traffic Control Plan");
    requirementArray.push("Site Plan");
...
function getDocReqsBlanket() {
    requirementArray = new Array();
    missingDocsArray = new Array();
  
    requirementArray.push("Traffic Control Plan");
    requirementArray.push("Insurance Documents");
...
function clearLPData(dailyComponentName)
{
		...
		var licenseProfessionalModel = cap.getLicenseProfessionalModel();
