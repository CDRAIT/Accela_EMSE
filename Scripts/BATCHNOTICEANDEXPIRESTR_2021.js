/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticeAndExpireSTR_2021  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 01/22/2021 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Script is executed one time on 05/31/2021
|
| Criteria:
|   Application is has a status of 'Pending Renewal"
|   ASI Permit 'Expiration Date' prior to 12/31/2021
|   appGroup = "ShortTermRental"
|   appPerType = "Short Term Rental"
| 
| Updates: TDunn 01/25/2021 created.
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var ccEmailAddrAdmin = "tdunn@truepointsolutions.com";  
...
var paramsAppStatusArray = new Array("Pending Renewal");
...
var paramsEndDt = aa.date.parseDate("12/30/2021");                                // End Date for the batch script to select records on Expiration Date.
...
if (paramsOK) {
    ...
    logMessage("End of Job: batchNoticeAndExpireSTR, Elapsed Time : " + elapsed() + " Seconds.");
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			aa.print("record number = " + capIDString + ". Expiration date: " + permitExpireDate + ". Cap status: " + capStatus);
			var emailTemplate = "NOTICE_STR_PERMIT_HAS_EXPIRED_ONE_TIME";
			...
			aa.print("Email results = " + emailResult)
			if(emailResult) {
