...
CAPIDS.push(expResult4[i])
    ...
    for (x in CAPIDS) // for each b1expiration (effectively, each license app) 
    {
       ...
       for (iNum in iList) 
	   {
	   var Balance = Number(getinvoicebalance(iList[iNum].getCustomizedNbr(),capId));
	   var duedate = dateAdd(dateFormatted(iList[iNum].getInvDate().getMonth(),iList[iNum].getInvDate().getDayOfMonth(),iList[iNum].getInvDate().getYear(),""),30); // used for when to generate report / due date below, should be set to 45 11/19/19, changed to 30 days, blame sf
	   var today = sysDate.getMonth() + "/" + sysDate.getDayOfMonth() + "/" +sysDate.getYear();
	   var letter = getAppSpecific("First Letter Sent",capId);
	   var ICContact = getContactEmailByContactType("Invoice Contact",capId);
       var ROContact = getContactEmailByContactType("Responsible Official",capId);
	   var temail = "sfrancis@placer.ca.gov";
	   var notestring = "";
	   if(ICContact != "" && ROContact != "")
	   {
	   ...
	   if( Balance > 0 && new Date(duedate) <= new Date (today) && letter != "CHECKED")
{
                 paramMap = aa.util.newHashMap();
				 paramMap.put("InvoiceNumber",String(iList[iNum].getInvNbr()));
				 emailParameters = aa.util.newHashtable();
				 addParameter(emailParameters,"$$INVOICE$$",String(iList[iNum].getInvNbr()));
				 //report = generateReport("Past Due Invoices - 1st Letter_to_Disk",paramMap,"AirQuality",capId);//for testing
				 //sendtest = sendNotification("pcapcd@placer.ca.gov","ngraf@truepointsolutions","","AQ 1ST LETTER",emailParameters,new Array(report),capId); //for testing
				 report = generateReport("Past Due Invoices - 1st Letter_to_Disk",paramMap,"AirQuality",capId);
				 sendtest = sendNotification("pcapcd@placer.ca.gov",temail,"sfrancis@placer.ca.gov;rmoore@placer.ca.gov","AQ 1ST LETTER",emailParameters,new Array(report),capId);
				 notestring = "1st Letter has been sent due on " + duedate;
				 if (sendtest == "true")
				 {
				 var arrEmission = new Array(); 
				 arrEmission["InvoiceNumber"] = String(iList[iNum].getInvNbr());arrEmission["Notes"] = String(notestring);arrEmission["Date Created"] = String(today);arrEmission["Created By"] = "ADMIN";
					editAppSpecific("First Letter Sent","CHECKED",capId);
					addToASITable("INVOICE COMMENTS",arrEmission,capId);
				 }
  logDebug("Invoice number = " + iList[iNum].getInvNbr() + " balance = " + Balance + " Invoice due date = " + duedate + " to email " + temail);
   logDebug(notestring);
