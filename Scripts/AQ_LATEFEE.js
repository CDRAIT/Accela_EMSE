...
if (paramsOK) {
    logMessage("START", "Start of Sending AQ_LATEFEES Batch Job.");

    var licAboutToExpCnt = aboutExpLics();

    logMessage("INFO", "Number of records processed: " + licAboutToExpCnt + ".");
    logMessage("END", "End of Sending AQ_LATEFEES Batch Job: Elapsed Time : " + elapsed() + " Seconds.");
}

if (emailAddress.length)
    aa.sendMail(senderEmailAddr, emailAddress, emailAddress2, batchJobName + " Results for Sending AQ_LATEFEES", emailText);
/*------------------------------------------------------------------------------------------------------/
| <===========END=Main=Loop================>
/------------------------------------------------------------------------------------------------------*/
...
//}
    ...
    for (x in CAPIDS) // 
    {
		...
		perid1  = CAPIDS[x].getID1();
		perid2  = CAPIDS[x].getID2();
		perid3  = CAPIDS[x].getID3();
//		logDebug("Facility:" + capId.getCustomID());
		...
		if (capDetailObjResult.getSuccess())
		{
			...
			var TotalBalance = capDetail.getBalance(); // get facility balance due total
		}
		var Balance = Number(TotalBalance) - Number(getlatefeebalance(capId));
		var BalanceDue = Number(Balance) * .5;
		...
		var firstletter = getAppSpecific("First Letter Sent",capId);
		var secondlettersent = getAppSpecific("Second Letter Sent",capId);
		var sendlatefees = getAppSpecific("Send Late Fees",capId);
		...
		var FACcapId = getCapId(perid1, perid2, perid3);
	   ...
	   if ( Balance > 0  && firstletter == "CHECKED" &&  secondlettersent != "CHECKED" && sendlatefees == "CHECKED")
		{
	        addFee("AQ_LATEFEES","AQ_FAC","FINAL",BalanceDue.toFixed(2) ,"Y",capId);

				 
				 var vAsyncScript = "AQ_ASYNC_LATE_INVOICE";
	
					var envParameters = aa.util.newHashMap();
					envParameters.put("ServProvCode","PLACERCO");			// Service Provider Code
					envParameters.put("CustomCapId",customID);			// Custom CAP ID
					envParameters.put("ThisCap", capId);			// CapId
					envParameters.put("Email",temail);
					logDebug("envParameters: " + envParameters);
					
					
	
					//call async script
					logDebug("Calling ASYNC");
					aa.runAsyncScript(vAsyncScript, envParameters);
					logDebug("Back from ASYNC");
...
//  logDebug("Invoice number = " + iList[iNum].getInvNbr() + " balance = " + Balance + " Invoice due date = " + duedate + " to email " + temail);
  ...
  capCount++;          
     }// end for loop for all facilities
    return capCount;
}
