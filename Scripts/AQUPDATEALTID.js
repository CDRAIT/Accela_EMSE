    ...
    for (i in myExp) // for each b1expiration (effectively, each license app) 
    {
		...
		var childcount = getChildrencount("AirQuality/Stationary Source/Permit to Operate/*",oldcapId);
		 ...
		 for(eachchild in capchildren)
		 {
             ...
             var newpermit = getAppSpecific("New Permit ID",childcap.getCapID());
			 
			 if(newpermit == null|| newpermit == "")
			 {
				 logDebug("Record AltID " + customID + " New Permit ID is blank");
			 }	
 
			if(newpermit != null && newpermit != "")
			 {
				 aa.cap.updateCapAltID(childcap.getCapID(),newpermit);
				 logDebug("Updated Record AltID from " + customID + " to " + newpermit);
			 }	
...
function getChildren(pCapType, pParentCapId) 
	...
	if (!getCapResult.getSuccess())
		{ logDebug("**WARNING: getChildren returned an error: " + getCapResult.getErrorMessage() + " " + vCapId); return null }
		
	var childArray = getCapResult.getOutput();
	if (!childArray.length)
		{ logDebug( "**WARNING: getChildren function found no children" + " " + vCapId); return null ; }
	...
	logDebug("getChildren returned " + retArray.length + " capIds" + " " + vCapId);
	return retArray;
