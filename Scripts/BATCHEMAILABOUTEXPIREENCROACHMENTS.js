/*------------------------------------------------------------------------------------------------------/
| Program: batchEmailAboutExpireEncroachments  Trigger: Batch    Client : Placer County
|
| Version 1.0 - Modified from BATCHEMAILEXPIRETRPA_RES script (by Terry Dunn). 08/18/2022, Ebrahim Aftahi
| Version 1.0.1- Changed getCapAddress() method to return an appropriate message on records without a primary address. 10/24/22, E. Aftahi
|
| Designed for EMSE 2.0
|
| Description of Intended Functionality: Script is executed daily. Tests for Expiration dates within the
| next 7 days.  Generates an email to contact using Notification Template.
|
| Criteria:
|   Permit is not closed, expired, withdrawn, cancelled.
|   ASI Expiration date is 7 days from 'today'.
|   appGroup = "PublicWorks"
|   appType = "Encroachment"
| 
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var paramsAppGroup = "PublicWorks";
var paramsAppPerType = "Encroachment";
var paramsAppSubTypeArray = new Array("NA", "NA");
var paramsAppStatusArray = new Array("Expired", "Construction Complete", "Void", "Withdrawn", "DONE", "On Hold");

var paramsAppSubGroupName = "EXPIRATIONS";                                         // Application Spec Info Subgroup Name that the ASI field is associated to.
var paramsAppSpecInfoLabel = "Expiration Date";                                    // ASI field name that the batch script is to search.
var paramsStartDt = aa.date.parseDate(dateAdd(null,7));                          // Start Date for the batch script to select records on Expiration Date.
var paramsEndDt = aa.date.parseDate(dateAdd(null,7));                            // End Date for the batch script to select records on Expiration Date.
...
var NotificationType = "ENCROACHMENT PERMIT NEARING EXPIRATION NOTICE";                                        // Email notification template name
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("START", "Start of Batch Job: batchEmailAboutExpireEcroachments.");
    ...
    logMessage("END", "End of Job: batchEmailAboutExpireEcroachments, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Emails Sent for Public Works Encroachment Permits that are about to Expire: " + emailsentCount;
}
...
function ExpireNotice() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && capPerType == paramsAppPerType && exists(capSubType, paramsAppSubTypeArray) && !exists(capStatus, paramsAppStatusArray)) {
            ...
            //sendNotification(senderEmailAddr, "eaftahi@placer.ca.gov", ccEmailAddr, NotificationType, params, null);       //for test
            capCount++;
        }
    }
...
function getCapAddress(capId) {
    ...
    if (capAddressResult1.getSuccess()) {
        ...
        /**
        * Abe - 10/20/22 
        * Reset the CapAddress to ""
        * In case of having no primary address it was returning prior address in the variable. 
        */
        if (CapAddress == "")
            CapAddress = "No Primary Address!";
        return CapAddress;
    }
}
