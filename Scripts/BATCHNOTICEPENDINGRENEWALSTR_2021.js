/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticePendingRenewalSTR_2021  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 12/01/2020 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Script is executed annually on 12/31/xxxx. 
|
| Criteria:
|   Application is in an active status and has been Issued or is in Pending Renewal
|   ASI Permit 'Expiration Date' prior to 12/25/current year
...
|          TDunn 02/10/2021 added new passWord parameter for pending renewal notification.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var batchJobName = "Batch: 2021 One time renewal notice";
var capId;
...
var emailAddrAdmin = "JMcKenzi@placer.ca.gov";     
var ccEmailAddrAdmin = "tdunn@truepointsolutions.com; jpaine@placer.ca.gov";  
...
var paramsAppStatusArray = new Array("Issued","Pending Renewal");
...
var paramsStartDt = aa.date.parseDate("12/01/2020");                         // Start Date for the batch script to select records on Expiration Date.
var paramsEndDt = aa.date.parseDate("12/25/2021");                           // End Date for the batch script to select records on Expiration Date.
...
if (paramsOK) {
    ...
    emailText = "Number of Short Term Rental permits processed " + expiredCount;
}
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
		...
		// aa.print("record number = " + capIDString + ". Expiration date: " + permitExpireDate + ". " + capGroup + capPerType);
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			// aa.print("record number = " + capIDString + ". Expiration date: " + permitExpireDate + ". " + "Status = " + capStatus + " Type: " +capGroup + capPerType);
			var emailTemplate = "NOTICE_STR_PERMIT_IN_RENEWAL";
			...
			var shortDesc = getShortNotes(capId);
			var totNum = getAppSpecific("TOT Registration Number",capId);
			var tempPW = thisTOT + capId3;
			aa.print("Record number = " + capIDString + ". Expiration Date: " + permitExpireDate + "; Temp password: " + tempPW + ". appSpec TOT: " + totNum);
			...
			addParameter(emailParameters,"$$passWord$$", tempPW);
			...
			aa.print("From: " + vFromEmail + " To: " + vToEmail + " ACA url: " + acaRecordUrl);
			aa.print("-------------------------------------------------------------------------");
