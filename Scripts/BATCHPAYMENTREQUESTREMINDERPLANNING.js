/*------------------------------------------------------------------------------------------------------/
| Program: batchPaymentRequestReminderPlanning  Trigger: Batch    Client : Placerco
|
| Version 1.0 - Base Version. 10/28/2022 Terry Dunn
|
| Designed for EMSE 3.0
|
| Description of Intended Functionality: Payment due notice sent to applicant each monday 
|
| Criteria:
...
|          EAFTAHI 11/09/2022 "Received" added to paramsAppStatusArray
|          EAFTAHI 12/09/2022 Changed acaRecordUrl parameter
| 
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var paramsStartDt = aa.date.parseDate(dateAdd(null,-90));                         // Start Date for the batch script to select records on Expiration Date.
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			if(isTaskStatus("Permit Initiation","Payment Requested") && isTaskActive("Permit Initiation")) {
				var emailTemplate = "NOTICE FEES DUE - REMINDER";
				...
				var shortDesc = getShortNotes(capId);
				logDebug("record number = " + capIDString + ". Last request date: " + lastRequestDate);
				...
				acaRecordUrl += "&TabName=" + capModule;
