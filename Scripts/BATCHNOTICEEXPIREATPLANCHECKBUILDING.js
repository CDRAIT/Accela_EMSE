...
|
| Description of Intended Functionality: Tests for Plan Check Expiration dates occuring in
|-1 days. Generates notice and sets cap status Expired permit
|
| Frequency: executes daily
|
| Criteria:
|   Permit is not issued, closed, expired, about to expire or other 'inactive' status
|   ASI Expiration date is in -1 days
|   appGroup = "Building"
...
var newCapStatus = "Expired"; 					                        // New Cap Status that will be applied. Not used for this script
var emailTemplate = "PLAN CHECK EXPIRED NOTICE";			// Email notification template name
...
if (paramsOK) {
    ...
    logMessage("End of Job: batchNoticeAboutToExpireAtPlanCheckBuilding, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "<u>Number of Building permits expiring at plan check <u>" + expiredCount + "<br><br>" +
...
*/			
            ...
            // Update App Status //
            updateAppStatus(newCapStatus, "Expired at Plan Check. Updated by Script", capId);
	        capCount++;
        }
    }
