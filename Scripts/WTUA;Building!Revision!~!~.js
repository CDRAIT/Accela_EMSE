/*--------------------------------------------------------------------------------------------------------------------------------/
| Program : WTUA;Building!Revision!~!~
| Event   : WorkflowTaskUpdateAfter
|
| Client  : Placer County, CA
| Usage   : Workflow Task Update After for all Building Residential records.
| wfProcess: BLD_20231116_REV
| 
|
| Requires: EMSE 3.0 and Standard Choice: "EMSE_EXECUTE_OPTIONS": "SCRIPT" to be Active.
|
| Notes   : TDunn   07/10/2024 Created script.
|         : TDunn   08/14/2024 added pre-issuance blocks
|         : TDunn   08/28/2024 resynced with Main building WTUA script updates.
|         : TDunn   09/04/2024 added setting preissuance due dates on Reconciliation complete
|         : TDunn   09/05/2024 added actions for Withdrawn
|         : TDunn   09/11/2024 updated plan review hours and fee assessment rules
|         : TDunn   09/12/2024 moved parent description and status update from 'Distribution Reconciliation' to 'Issuance'
|         : MHelvick 09/19/2024 added "moveDoc" functionality to 'Process for Issuance' - 'Approved' section
|         : TDunn   09/27/2024 added requirements for EER TSI related condition flags.
|         : TDunn   09/29/2024 added isPlacerFire flag and related activation logic.
|         : TDunn   10/03/2024 updated scope lookup to type + scope for additional record types
|         : TDunn  10/22/2024 added scripting for tracking review cycles based on TSI.
|         : TDunn  10/23/2024 added cycle tracking to Submittal Review.
|         : TDunn  10/24/2024 added cycle tracking to Distribution.
|         : TDunn  11/02/2024 added rules for new Stormwater and Flood Control TSI; added rules for new TSI on Public Works Review task
...
|         : TDunn  12/12/2024 added rules for updating Distribution Reconciliation task status when either Readiness/review tasks 
|                             where all approved or one or more had corrections.
|         : TDunn  12/12/2024 changed Process for Issuance task status 'Issued' to use 'Approved'. Updated for action criteria
...
|         : TDunn  12/21/2024 added new flags to support setting due dates, plan check type and staff assignment.
|         : TDunn  12/22/2024 added ESD addAppConditions rules based on TSI checkbox fields
|         : TDunn  02/20/2025 additions to task due dates and assignment rules
|         : TDunn  02/24/2025 additions to task due dates and assignment rules.
...
|         : TDunn  03/28/2025 added new notifications and rules (matching updates to WTUA:Building for 2/19 - 3/25)
...
logDebug("Running WTUA:Building/Revision ... ");
...
// Begin rules for Distribution logic for Revision
//=========================================================================
if(matches(appTypeArray[1],"Revision") && wfProcess == "BLD_20231116_REV") 
{
	logDebug("Running code for process BLD_20231116_REV");
	...
	if(getParent() != null && getParent() != false)
	{
		...
		dwSlope = getAppSpecific("Slope of Driveway",pCapId);
		includesDW = getAppSpecific("Includes Driveway",pCapId);
		logDebug("parentType = " + parentType + "; currentScope = " + currentScope);
	}

	// Setting variables for retrieving parent scope
	var varLookupTable = "SDL:Residential Scope of Work";
	var thisScope = currentScope;
	var thisType = parentType;
	...
	if(pappTypeArray[2] == "Limited") 
	{
		varLookupTable = "SDL:OTC Scope of Work";
		thisScope = thisType + "|" + currentScope;
		logDebug("Scope to this Res limited = " + thisScope);
	}
	if(pappTypeArray[3] == "Residential<3000") 
	{
		varLookupTable = "SDL:Residential 3000 Scope of Work";
		logDebug("Scope for this Res <3000 = " + thisScope);	
	}
	if(pappTypeArray[3] == "Residential>3000") 
	{
	...
	if(pappTypeArray[3] == "Solar App") {varLookupTable = "SDL:SolarApp Scope of Work";}
	if(pappTypeArray[2] == "Plan Check Only")
	{
		if(pappTypeArray[3] == "Master < 3000") {varLookupTable = "SDL:Residential Master<3000";}
		if(pappTypeArray[3] == "Master > 3000") {varLookupTable = "SDL:Residential Master>3000";}
	}
	if(pappTypeArray[1] == "Commercial")
	{
		comFire = true;
		if(pappTypeArray[2] == "Full Review") 
		{
	...
	//=====================================================================
	if(pappTypeArray[1] == "Residential")
	{
		if(pappTypeArray[2] == "Full Review") 
		{
			pcheckType = "Full";
	...
	if (matches(wfTask, "Distribution") && matches(wfStatus, "Not Required - Process for Issuance"))
	{
		...
		resubNum = AInfo["Resubmittal Number"];
		if(resubNum <= 1) {assignTask(thisTask,thisStaff,wfProcess);}
		if(resubNum > 1)
		{		
	...
	{	
		...
		if(matches(wfStatus,"Approved","Approved Pending Resubmittal"))
		{
			...
			// restore capId to current revision record
			capId = saveCapId;
		}
	}
	...
	if(matches(wfTask,"Building Plan Check"))
	{
		vTotHrs = 0;
		...
		if(wfStatus == "Approved Pending Resubmittal")
		{
			if(matches(AInfo["LPG Tank"],"Above Ground","Underground"))
			{
				...
				{
					addStdCondition("Fire - Prevent Final / Completion","Fire LPG Article 15.12 Inspections Required",pCapId);
				}
				if(AInfo["LPG Tank"] == "Above Ground" && !feeExists("915","NEW","INVOICED"))
				{
					updateFee("9015","FIRE PLANNER FEES","FINAL",1,"N");
				}
				if(AInfo["LPG Tank"] == "Underground"&& !feeExists("914","NEW","INVOICED"))
				{
	...
	if(matches(wfTask,"Stormwater and Floodplain Review"))
	{
		if(matches(wfStatus,"Approved","Approved Pending Resubmittal"))
		{
			saveCapId = capId;
			capId = pCapId;
			logDebug("Inside Stormwater and Floodplain Review");
			if(matches(AInfo["Required before Foundation"],"Y","Yes","YES"))
			{
				logDebug("Required before Foundation = " + AInfo["Required before Foundation"]);
				if(!appHasCondition("DPW - Prevent Building Inspections", "Applied","DPW Prevent Building Inspections", null))
				{
					logDebug("Attemping to add condition");
			...
			{
				logDebug("Required before Final = " + AInfo["Required before Final"]);
	...
	// Cycle updates for all review tasks ------------
