/*------------------------------------------------------------------------------------------------------/
| Program: batchEmailPlanCheckExpireTRPA_Res  Trigger: Batch    Client : Placer County
|
| Version 1.0 - Base Version. 11/20/2013, Terry Dunn TPS
| Version 2.0 - 
| Version 2.2 - Corrected error in ASI field name for getAppSpecific.  Modified criteria for qualifying 
|               record to using only one date compare and modified tests for app status and subType
| Version 3.0 - Script logic completely restructured for more efficient processing.  Permit Expiration 
|               date and Plan Check Separated into separate batches.
|
| Designed for EMSE 2.0
|
| Description of Intended Functionality: Script is executed daily. Tests for Expiration dates 30 days from 
| today.  Generates an email to contact using Notification Template.
|
| Criteria:
|   Permit is not closed, expired, withdrawn, cancelled.
|   ASI Plan Check Expiration is 30 days from 'today' and Expiration Date is null.
|   appGroup = "Building"
|
| Modified By: Joe Cipriano, TPS, 01/30/2015 - This a new batch scripted created from batchEmailAboutToExpireCom.
|                                              Batch script to help with timeout error and to only process one expiration
|                                              date at a time.
|                                            - Batch run time changed to run for 10 mins.
...
var paramsAppPerType = "Building";
var paramsAppSubTypeArray = new Array("Qualified Exempt", "TRPA MOU Project");
...
var paramsStartDt = aa.date.parseDate(dateAdd(null,+60));                          // Start Date for the batch script to select records on Expiration Date.
...
var newCapStatus = "About to Expire"; 					           // New Cap Status that will be applied.
var NotificationType = "PLAN CHECK NEARING EXPIRATION NOTICE";                        // Email notification template name
...
if (paramsOK) {
    var totalCount = 0;
    logMessage("START", "Start of Batch Job: batchEmailPlanCheckExpireTRPA_Res.");

    var emailsentCount = ExpireNotice();

    logMessage("INFO", "Sent " + emailsentCount + " Emails. System Date : " + new Date(paramsCurrentCompareDt).valueOf());
    logMessage("END", "End of Job: batchEmailPlanCheckExpireTRPA_Res, Elapsed Time : " + elapsed() + " Seconds.");
    emailText = "Emails Sent for TRPA Building Permits that are about to Expire at Plan Check: " + emailsentCount;
}
...
function ExpireNotice() {
    ...
    for (i in csmArray) {
        ...
        // get associating cap information
        var planCheckExpireDate = getAppSpecific("Plan Check Expiration", capId);
        var permitExpireDate = getAppSpecific("Expiration Date", capId);

        // Process records if condition is met //
        if (capGroup == paramsAppGroup && capPerType == paramsAppPerType && exists(capSubType, paramsAppSubTypeArray) && !exists(capStatus, paramsAppStatusArray) && permitExpireDate == null) {

            // Set Variables //
            var scopeCode = getAppSpecific("Scope of Work", capId);
            var planCheckOrPermit = "at Plan Check on"; // Email text for when expiration date is for Permit or Plan Check
            ...
            params.put("$$PERMITEXPIREDATE$$", planCheckExpireDate);
            ...
            // sendNotification(senderEmailAddr, "tdunn@truepointsolutions.com", ccEmailAddr, NotificationType, params, null); // Remark out line above and unremark this line for testing. 
            capCount++;
        }
    }
