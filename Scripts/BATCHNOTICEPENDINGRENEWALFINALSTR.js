/*------------------------------------------------------------------------------------------------------/
| Program: batchNoticePendingRenewalFinalSTR  Trigger: Batch    Client : Placerco
|
...
var paramsEndDt = aa.date.parseDate("12/25/2021");    
...
function ExpiringPermit() {
    ...
    for (i in csmArray) {
        ...
        if (capGroup == paramsAppGroup && exists(capPerType, paramsAppPerTypeArray) && exists(capStatus, paramsAppStatusArray)) {
			var emailTemplate = "NOTICE_STR_PERMIT_ABOUT_TO_EXPIRE_FINAL";
			...
			test = getParcel(capId);
			for(x in test) {
				aa.print(test[x].getParcelNumber());
				thisParcelID = test[x].getParcelNumber();
			}
			...
			addParameter(emailParameters,"$$parcelID$$", thisParcelID);
...
function getParcel(capId)
{
               ...
               {
                              capParcelArr = s_result.getOutput().toArray();
