//A variatiaon parcelConditionExists() created by Abe for PCCP parcel conds
function parcelConditionExists_PCO(thisCondType, thisCondDesc, thisCondStatus) {
	...
	for (zz in Parcels) {
		...
		for (pc1 in pcs)
			if (pcs[pc1].getConditionType().equals(thisCondType) && pcs[pc1].getConditionDescription().equals(thisCondDesc) && pcs[pc1].getConditionStatus().equals(thisCondStatus))
				return true;
	}
}